...
--
-- RLS de esta fase (F0) es ADITIVA y NO destructiva. IMPORTANTE: se escribio para
-- coincidir con el patron REAL de RLS en produccion (verificado por SQL directo):
--   - admin = profiles.org_role IN ('owner','admin')  (NO existe is_org_admin()).
--   - membresia de workspace = subquery inline a workspace_members (role es text).
--   - auth_org_id() SI existe y se reutiliza.
-- La policy extra de SELECT sobre `notes` se agrega sin tocar la existente
-- (Postgres OR-combina permissive). El ocultamiento estricto de espacios
-- restringidos se termina en F3.
-- ════════════════════════════════════════════════════════════════════════════
...
-- ── Helpers especificos de space (SECURITY DEFINER, search_path fijo) ───────
-- Se crean DESPUES de las tablas porque el cuerpo SQL valida las referencias en
-- tiempo de creacion. No se crean helpers globales (is_org_admin, etc.) para no
-- colisionar con la logica global; se usa subquery inline en las policies.
CREATE OR REPLACE FUNCTION is_space_member(sp_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$ SELECT EXISTS (SELECT 1 FROM space_members WHERE space_id = sp_id AND profile_id = auth.uid()) $$;

CREATE OR REPLACE FUNCTION is_space_admin(sp_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$ SELECT EXISTS (SELECT 1 FROM space_members WHERE space_id = sp_id AND profile_id = auth.uid() AND role IN ('owner','admin')) $$;
...
CREATE POLICY "spaces_select" ON spaces FOR SELECT USING (
  ...
  AND (
    ...
    OR (is_restricted = false AND workspace_id IN (
...
CREATE POLICY "spaces_update" ON spaces FOR UPDATE USING (
  ...
  AND ((SELECT org_role FROM profiles WHERE id = auth.uid()) IN ('owner','admin')
       OR is_space_admin(id) OR created_by = auth.uid())
);
...
CREATE POLICY "space_members_delete" ON space_members FOR DELETE USING (
  (SELECT org_role FROM profiles WHERE id = auth.uid()) IN ('owner','admin')
  OR is_space_admin(space_id) OR profile_id = auth.uid()
);
...
CREATE POLICY "notes_select_space" ON notes FOR SELECT USING (
  space_id IS NOT NULL
  AND (
    (SELECT org_role FROM profiles WHERE id = auth.uid()) IN ('owner','admin')
    OR is_space_member(space_id)
    OR (workspace_id IN (SELECT workspace_id FROM workspace_members WHERE profile_id = auth.uid())
        AND EXISTS (SELECT 1 FROM spaces s WHERE s.id = notes.space_id AND s.is_restricted = false))
  )
);
