...
export default function FlowEditor({flowId,workspaceSlug,initialNodes,initialEdges,initialTitle,initialDescription}:Props){
  ...
  const onConnect=useCallback((conn:Connection)=>{setEdges(eds=>{const u=addEdge({...conn,style:{stroke:'#64748b',strokeWidth:2},markerEnd:{type:MarkerType.ArrowClosed,color:'#64748b'}},eds);autoSave(nodes,u);return u})},[setEdges,nodes,autoSave])
  const addNode=useCallback((ct:NodeContent['contentType']='text')=>{const id=`node-${Date.now()}`;setNodes((nds:any[])=>{const u=[...nds,{id,type:'custom',position:{x:Math.random()*400+100,y:Math.random()*300+100},data:{label:'Nuevo nodo',content:{contentType:ct,content:''}}}];autoSave(u,edges);return u})},[setNodes,edges,autoSave])
  ...
  const onDrop = useCallback((e:React.DragEvent)=>{
    ...
    if(!type)return
    if(type==='text'||type==='html'||type==='url'||type==='document'){const id=`node-${Date.now()}`;setNodes((nds:any[])=>{const u=[...nds,{id,type:'custom',position:pos,data:{label:'Nuevo nodo',content:{contentType:type,content:''}}}];autoSave(u,edges);return u})}
    else{const id=`shape-${Date.now()}`;const shape=type as ShapeType;const dims=shape==='line'?{width:200,height:40}:shape==='grid'?{width:240,height:200}:shape==='text'?{width:160,height:50}:{width:160,height:120};setNodes((nds:any[])=>{const u=[...nds,{id,type:'shape',position:pos,data:{shape,...dims,label:shape==='text'?'Nuevo texto':'',fill:'#f1f5f9',stroke:'#64748b',rows:3,cols:3}}];autoSave(u,edges);return u})}
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute bg-card border rounded-lg shadow-lg z-20 transition-all" style={{right:toolPos.x||12,top:toolPos.y||60,width:toolCollapsed?40:180}}>
          ...
          {!toolCollapsed&&<div className="flex flex-col gap-1 p-2">
            ...
            <button className="inline-flex items-center justify-start gap-1.5 rounded px-1.5 py-1 text-xs hover:bg-accent transition-colors cursor-grab" draggable onDragStart={e=>{e.dataTransfer.setData('application/reactflow','rect');e.dataTransfer.effectAllowed='move'}} ><Square className="w-3.5 h-3.5"/>Rectangulo</button>
            <button className="inline-flex items-center justify-start gap-1.5 rounded px-1.5 py-1 text-xs hover:bg-accent transition-colors cursor-grab" draggable onDragStart={e=>{e.dataTransfer.setData('application/reactflow','circle');e.dataTransfer.effectAllowed='move'}} ><Circle className="w-3.5 h-3.5"/>Circulo</button>
            <button className="inline-flex items-center justify-start gap-1.5 rounded px-1.5 py-1 text-xs hover:bg-accent transition-colors cursor-grab" draggable onDragStart={e=>{e.dataTransfer.setData('application/reactflow','line');e.dataTransfer.effectAllowed='move'}} ><Minus className="w-3.5 h-3.5"/>Linea</button>
            <button className="inline-flex items-center justify-start gap-1.5 rounded px-1.5 py-1 text-xs hover:bg-accent transition-colors cursor-grab" draggable onDragStart={e=>{e.dataTransfer.setData('application/reactflow','grid');e.dataTransfer.effectAllowed='move'}} ><Grid3X3 className="w-3.5 h-3.5"/>Cuadricula</button>
            <button className="inline-flex items-center justify-start gap-1.5 rounded px-1.5 py-1 text-xs hover:bg-accent transition-colors cursor-grab" draggable onDragStart={e=>{e.dataTransfer.setData('application/reactflow','text');e.dataTransfer.effectAllowed='move'}} ><Type className="w-3.5 h-3.5"/>Texto</button>
            <hr className="my-0.5"/>
