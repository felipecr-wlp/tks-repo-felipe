...
export default function FlowEditor({flowId,workspaceSlug,initialNodes,initialEdges,initialTitle,initialDescription}:Props){
  ...
  const copySelected=useCallback(()=>{const sel=nodes.filter((n:any)=>n.selected);clipboard.current=sel.map((n:any)=>({...n,id:`node-${Date.now()}`}));toast.success(`${sel.length} copiado${sel.length!==1?'s':''}`)},[nodes])
  ...
  const onKeyDown=useCallback((e:React.KeyboardEvent)=>{
    ...
    if(e.key==='Delete'||e.key==='Backspace'){deleteSelected();pushHistory(nodes,edges)}
    else if(e.ctrlKey&&e.key==='z'){e.preventDefault();undo()}
    else if(e.ctrlKey&&e.key==='y'){e.preventDefault();redo()}
    else if(e.ctrlKey&&e.key==='c'){e.preventDefault();copySelected()}
    else if(e.ctrlKey&&e.key==='x'){e.preventDefault();copySelected();deleteSelected();pushHistory(nodes,edges)}
    else if(e.ctrlKey&&e.key==='v'){e.preventDefault();pasteSelected()}
  },[deleteSelected,undo,redo,copySelected,pasteSelected,nodes,edges,pushHistory])
  ...
  const deleteSelected=useCallback(()=>{setNodes((nds:any[])=>{const sel=nds.filter((n:any)=>n.selected);const rest=nds.filter((n:any)=>!n.selected);const ids=new Set(sel.map((n:any)=>n.id));setEdges(eds=>{const u=eds.filter(e=>!ids.has(e.source)&&!ids.has(e.target));autoSave(rest,u);return u});pushHistory(rest,edges.filter((e:any)=>!ids.has(e.source)&&!ids.has(e.target)));return rest})},[setNodes,setEdges,autoSave,edges,pushHistory])
  ...
  return (
    <div className="flex flex-col h-full" tabIndex={0} onKeyDown={onKeyDown}>
