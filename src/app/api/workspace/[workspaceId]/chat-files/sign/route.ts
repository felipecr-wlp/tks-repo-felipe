/**
 * GET /api/workspace/[workspaceId]/chat-files/sign?paths=<a>,<b>,…
 * Resuelve signed URLs temporales para archivos del canal General de un
 * workspace. Los paths viven en la columna attachments jsonb del mensaje; como
 * las signed URL caducan, el cliente las pide en vivo al render y las re-firma
 * cuando expiran.
 *
 * Seguridad:
 *  - Auth + canAccessWorkspaceById.
 *  - Cada path DEBE empezar con workspace/<workspaceId>/ (anti-IDOR: no se firma
 *    nada de otro workspace aunque el path venga manipulado).
 */
...
export async function GET(
  ...
  const prefix = `workspace/${params.workspaceId}/`
